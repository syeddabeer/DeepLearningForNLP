...
# from keras.proprocessing.text import Tokernizer
# from keras.preprocessing.sequence import pad_sequences
# tokenizer=Tokenizer(num_words=max_words)
# tokenizer.fit_on_texts(texts)
# word_index=tokenizer.word_index
# sequences=tokenizer.texts_to_sequences(texts)
# data=pad_sequences(sequences, max_len=max_len)
# labels = np.asarray(labels)
# onehotvector=tokenizer.texts_to_matrix(texts)
...
def preprocess_sentence(sentence):
    ...
    # lemmatize
    # print("processed_tokens: ", processed_tokens[1:10])
    processed_tokens = [lemmatizer.lemmatize(w) for w in processed_tokens]
    # print("processed_tokens lemmatized: ", processed_tokens[1:10])
